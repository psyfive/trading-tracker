...
(30주선 위 + 30주선 상승이 이어진 연속 일수). 이 값은 워밍업 구간에서 잘리므로
**관측 가능한 하한**이다 — 실제로는 더 오래됐을 수 있다.
...
def stage2_age_days(ctx: StockContext, config: WeinsteinConfig) -> int | None:
    ...
    이동평균이 아직 산출되지 않은 워밍업 구간은 조건이 False로 떨어지므로,
    반환값은 관측 가능한 **하한**이다 (데이터가 더 있었다면 더 컸을 수 있다).
    반대 방향으로 틀리지 않는다는 점이 중요하다 — 신선도를 과대평가하지 않는다.
    """
    ...
    in_stage2 = ((close > ma) & (slope > config.slope_min_pct)).to_numpy()
    age = 0
...
def detect_range(ctx: StockContext, config: WeinsteinConfig) -> TradingRange | None:
    ...
    avg_volume = ctx.indicators.avg_volume_50
    peak_volume = float(trading_range["volume"].iloc[-recent_span:].max())
    ...
    return TradingRange(
        ...
        breakout_volume_ratio=(peak_volume / avg_volume if avg_volume else None),
    )
...
class WeinsteinStrategy(StrategyBase):
    ...
    def build_score(self, ctx: StockContext) -> tuple[float, float, list[ScoreComponent]]:
        cfg = self.config
        ind = ctx.indicators
        trading_range = detect_range(ctx, cfg)
