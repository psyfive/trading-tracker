...
## sample_watchlist.json — 두 번째 루트 계약

`WatchlistReport`다. 유니버스 29종목을 **`sample_buy.json`과 같은 날짜**로 스캔한
결과이므로, 목록에서 그 티커 줄을 펼치면 `sample_buy.json`이 나온다 — 프론트는 이 두
파일만으로 '목록 -> 상세' 이동을 만들 수 있다.

- `entries`는 **정렬된 상태로 온다**: `(BUY 전략 수, 게이트 진행률)` 내림차순.
  정렬은 계약이므로 화면에서 다시 정렬하지 말 것. 필터(상위 N, 판정)는 표시 단계의
  자유지만, 몇 개를 걸렀는지 함께 보여줘야 한다.
- `strategies[]`는 표 한 줄 분량의 **요약**이다. 게이트 체크 목록·점수 항목·셋업
  수치는 없다 — 그것이 필요하면 그 티커를 진단한다.
- `score_pct`는 만점 대비 비율이다. 게이트 탈락이면 `null`이고, 0%로 그리면
  '낮은 점수'로 읽힌다. 전략 간 비교는 여전히 하지 말 것 (척도가 다르다).
- `failed[]`는 진단하지 못한 종목과 이유다. 비어 있지 않으면 `requested`가
  `entries` 수보다 크다 — 그 차이를 화면에 남겨야 '전부 훑었다'는 말이 참이 된다.
