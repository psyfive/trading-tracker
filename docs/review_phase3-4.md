...
## Phase 6에서 답이 나온 항목 (2026-08-20)

리뷰가 '조치하지 않았고, 결정이 필요하다'로 남긴 세 항목을 학습/홀드아웃 분할 위에서
측정했다. 상세는 `docs/phase6_sweep.md`, 재현은 `python scripts/verify_phase6.py`.

| 항목 | 측정 결과 | 조치 |
|---|---|---|
| Qullamaggie SCORE 역전 | 점수 기준을 올릴수록 학습 초과수익이 단조 **악화**(+0.41 → -11.90%p), 홀드아웃은 임계값과 무관하게 평평(-1.7~-2.6%p) | **임계값으로 고칠 문제가 아님**을 확인. 채점 항목 재설계가 필요하며 그것은 이 표본에 맞춘 가중치 조정이 아니라 방법론 재해석의 몫이다 |
| 와인스타인 게이트 느슨함 | `min_rs_percentile` 50→80에서 학습·홀드아웃 **양쪽 단조 개선**(홀드아웃 -1.70 → +1.10%p). 다만 학습 argmax(70)와 2위(80)의 차이가 표준오차 안이라 값 선택은 불가 | **유지.** 방향성은 가장 유력한 변경 후보로 남기되, 3년 단일 표본으로 임계값을 바꾸지 않는다 |
| PIVOT_READY 진입 정의 | `require_breakout_for_buy=True`면 진입이 급감(minervini 91→8, qullamaggie 25→1)해 학습 표본이 기준 미달. 표본이 남은 와인스타인은 학습 +0.54 → 홀드아웃 -2.12로 붕괴 | **유지.** 플래그는 config에 남겨 표본이 갖춰지면 다시 잰다 |

이 Phase는 **임계값을 하나도 바꾸지 않았다.** 그것이 결과다 — 이전에는 근거 없이 바꿀
수 있었고, 지금은 바꾸려면 홀드아웃을 통과해야 한다. 특히 와인스타인 RS 기준은 도구가
없었다면 "학습에서 70이 최고"로 끝나 홀드아웃 -0.13%p짜리 변경을 개선이라 불렀을 것이다.
